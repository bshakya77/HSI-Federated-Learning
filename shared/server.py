    ...
    def aggregate_fit(self, server_round, results, failures):
        ...
        if self._client_metrics_logger is not None and self._last_global_ndarrays is not None and results:
            ...
            for client_proxy, fit_res in results:
                ...
                alpha = float(m.get("attack_alpha", 0.0)) if "attack_alpha" in m else 0.0
            ...
            for c in per_client:
                self._client_metrics_logger.info(
                    "[ROUND %s] client=%s malicious=%s alpha=%.3f delta_norm=%.6f pre_norm=%.6f post_norm=%.6f upd_cos=%.6f train_loss=%.6f train_mse=%.6f train_sam=%.6f",
                    server_round,
                    c["cid"],
                    c["is_malicious"],
                    c["attack_alpha"],
