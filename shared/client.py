...
class HSIClient(fl.client.NumPyClient):
    ...
    def fit(self, parameters, config):
        ...
        alpha = float(config.get("sign_flip_alpha", 1.0))
        ...
        if is_malicious:
            ...
            for global_param, local_param in zip(global_params, updated_params):
                delta = local_param - global_param
                poisoned_delta = -alpha * delta
                poisoned_params.append(global_param + poisoned_delta)
        ...
        logging.info(
            "[Client %s] malicious=%s alpha=%.3f pre_norm=%.6f post_norm=%.6f cosine=%.6f nan_detected=%s nan_stage=%s",
            self.cid,
            is_malicious,
            alpha if is_malicious else 0.0,
            pre_update_norm,
            post_update_norm,
            update_cosine,
            nan_detected,
        ...
        return (
            ...
            len(self.trainloader.dataset),
            {
                ...
                "attack_alpha": float(alpha if is_malicious else 0.0),
